...
There is a set of Machines which can process Jobs. The Jobs are of different types and sizes. For the JobTypes, let's just say there is Type A, B, and C. Each machine has different JobTypes that it can handle. Some Machines can process any JobType while other machines can only work on one or two JobTypes. At the beginning of the day we are given a set of Jobs to assign to the Machines. We want to assign jobs to Machines such that a) the Machines are evenly loaded and b) we minimize the number of different JobTypes each machine has to process.

An ideal plan would have each Machine with the exact amount of work and only processing a single JobType. The reason we want a Machine to only process a single JobType is to minimize the wast associated with changing between JobTypes. Switching between JobTypes is fast, it just creates unwanted waste. Let's start with creating a small domain model and generate some example data.

> **Note**: You can find the full code example for this post [here]()

## A Domain for Job Assignments

Based on the description there are some pretty clear types that we need to define: `Machine`, `Job`, and `JobType`.
...
I like to override the `ToString` method because the `ConstraintBuilder` and `DecisionBuilder` use `ToString` for the naming of constraints and decisions. `JobType` is a straightforward Discriminated Union with three different cases `A`, `B`, and `C`. `Job` has an `Id` field for identifying a particular `Job`, a `JobType` which describes the type of job that it is, and the `Length` which how long it takes a `Machine` to process it.

The `Machine` type has an `Id` field uniquely identify it and a `JobTypes` field. The `JobTypes` field is a `Set` of `JobType`. This represents the jobs the `Machine` is able to process.

We now want to setup some data for us to be able to play with. These will be parameters which will help us generate random data for us to work with.
...
We can now generate a random set of `Job`s and `Machine`s to work with.
...
Now that we have some data to work with, we can get to formulating our model. Let's go ahead and open up `Flips`. I love working with VS Code, Ionide, and `.fsx` files for this kind of exploration. The new `#r "nuget: <library name>` syntax for using Nuget packages has been a game changer.
...
We want to create a `Map` where they key is a `JobType` and the value is a list of `Job`s that are of that type. This will make it easy for us to lookup `Job`s of a given type.
...
Now let's create the set of `Decision`s which will represent us assigning a `Job` to a `Machine`. We will store this in a 3-dimensional `SliceMap` indexed by the `Machine`, the `JobType`, and finally the `Job`. The reason we index by the `JobType` will become apparent later in the formulation.
...
We now come to an interesting question. Our original problem statement said that we want to minimize the number of different job-types a machine has to deal with. Ideally each machine only works on a single job-type. We also said that we want the machines evenly loaded. When I was chatting with my friend I dug into this point. Which one of these objectives is actually more important because can't optimize for both? This is where a modeler needs to work with their client to help them understand what is truly the most important thing.
...
We do a similar thing for the `minWork` decision. In this case we will say that `minWork` must be less than or equal to all of the loadings of the machines.
...
Let's bring it all together in a model.
...
If you want to see the code that prints out the results you can check it out [here](). This is the solution that solver found.
...
We've actually only begone to explore this model. There are quite a few variations and nuances that I will dive into in the posts to come. In the future we will discuss adding machine capacity and discuss dealing with infeasible models. We will also explore adding restrictions on just how much of a job-type can be assigned to a given machine. Some job-types cause more wear and therefore we do not want too much assigned to a single machine. We will also look at needing to re-plan part way through the day and look at scheduling over a longer time horizon.

These types of schedulign problems are common and therefore it's valuable to explore how we can play and tweak with this model to make it suit our needs. Feel free to reach out with questions and ideas for modeling challenges in the future!