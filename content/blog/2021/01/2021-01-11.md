...
I was recently posed the question, "Can you use Mathematical Planning to optimize the Cutting Stock problem?" For those who are not familiar with this problem, you can find the Wikipediat post [here](https://en.wikipedia.org/wiki/Cutting_stock_problem). This is a classic planning problem that can actually be reduced to the [knapsack problem](https://en.wikipedia.org/wiki/Knapsack_problem). In summary, you have a stock size of paper material from which you need to produce smaller sizes. In the example provided on Wikepedia, the stock size is 5600mm. You are asked to produce a variety of sizes between 1380mm and 2200mm. Ideally you find a plan which minimizes the waste.
...
There are a variety of different ways you can cut the stock size into the smaller sizes. For example, you could produce 3 1820mm cuts from a 5600mm stock roll. You could also do 200mm, 1820mm, and 1560mm. In total there are 308 possible combintations of cuts. The most important thing to realize when approaching this problem is that the order you make the cuts does not matter. A more technical term would be that the order of cuts is [Commutative](https://en.wikipedia.org/wiki/Commutative_property).
...
A `Cut` is a length we want to create from our stock rolls. A `Plan` is a set of cuts. We want an algorithm which will generate the possibles `Plan`s for a set of `Cut`s and a length for our stock roll. To make our lives easier, I am going to go ahead and write some functions which allow us to work with these types more easily. I find this incredibly helpful when formulating problems. Small, simple domains are nice to work with.
...
If the list of cuts is empty, as indicated by the `[]` case, then we add `plan` to `approved`. Now let's look at the case where there are `Cut`s remaining.
...
The optimization model for this is actually rather simple. We will create the list of possible `Plan`s using the function we just described. We will associate an integer `Decision` which each `Plan` which is to indicate how many of each of those plans will schedule. Let's setup the data for our model so that we can build it. All of this data is taken from the Wikipedia example.
...
We know want to start building our model. We'll open the namespaces we need and create our set of `Decision` associated with each `Plan` in `plans`. We are using [SliceMaps](http://matthewcrews.com/SliceMap/#/) to simplify formulation.
...
We then need to calculate the amount of each `Cut` that is associated with each `Plan`. This will be imporant for us to fromulate the constraints around meeting the minimum cut requirements. We will store this information in a 2-D `SliceMap` where the first index is the `Plan` and the second index is the `Cut`. The value in the `SliceMap` is the number of a given `Cut` in the `Plan`.
...
Our objective is to minimize the total number of cuts required to meet the demand for each `Cut`.
...
If you check the Wikipedia article, you will see that the best possible answer is 73. There are actually multiple, equally good solutions. This is called Degeneracy. Problems with high levels of Degeneracy can be difficult to solve but fortunately this one was not difficult. You may run this code on your machine and get a different set of plans, but you'll still have a total of 73.

## Next Steps

I hope this was interesting. This was a fun challenge and was a bit of a brain teaser. These types of problems are everything in manufacturing planning and schedule. Minimizing the amount of raw resources required is incredibly important but can be brutally difficult. It's often done by domain experts spending hours with Excel finding a plan that meets all the requirements. These are some of my favorite problems to turn into Mathematical Planning models. Thank you for your time and I look forward to chatting next week!