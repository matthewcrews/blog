...
As I continue my journey of growing as a developer, I am consistently inspired by language features which seem incredibly simple but yield incredible benefit. As I continue to learn F#, I am frequently surprised by how powerful the language is for expressing ideas while having so few features. Discussions frequently pop up about the need for ever more powerful abstractions, yet I find myself pleasantly amazed by how far you can take the language with what is already there.
...
Many of my colleagues are building their models with Python. Python is a great language and I have been productive with it in the past. I believe that F# provides some additional features that are of great value to my peers. Here is a snippet of what a mathematical planning model may look like in Python:
...
This is only part of the problem. Normally there would be normally be more constraints that would make it more interesting. This is enough of a problem to illustrate my case though. There are two errors in this model already. If you were paying close attention you may have found one. I promise you cannot detect the second.
...
We will create the Decisions for each `item` and location. We will store these `Decision` types in a `Map` which is indexed by a `(Item * Location)` tuple.
...
What some of you may have noticed in the Python code is that the `allocation` collection is indexed by an `Item` then `Location`. The original code was trying to access it by `location` then by `item`. This would have thrown an error at runtime due to a missing value. In F# though, this becomes a compiler error. The type system itself it is helping you. This may seem small, but this is one of the most painful types of errors when debugging a Mathematical Planning model.

## Units of Measure: The Achilles Heel of float
...
The important part of this message is at the bottom. What it is complaining about is that the left-hand side the units are in `Serving` and on the right-hand side the units are in `Kg`. It does not make sense to compare values that are in different units, so the compiler is throwing an error. In other languages this error would go undetected. Worse, it may not even be caught in unit testing because the math will still work, it just won't give correct results.
